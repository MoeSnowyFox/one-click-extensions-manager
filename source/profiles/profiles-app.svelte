<script lang="ts">
	...
	const GITHUB_URL =
		'https://github.com/MoeSnowyFox/one-click-extensions-manager';
	...
	async function initializeDefaultProfile(): Promise<void> {
		...
		const extensionStates: ExtensionStateConfig[] = allExtensions
			...
			.map(ext => ({
				extensionId: ext.id,
				targetState: ext.enabled
					? 'enable'
					: ('disable' as ExtensionTargetState),
			}));
	...
	function initDefaultExtensionStates(): void {
		const stateMap = defaultProfile
			? new Map(
					defaultProfile.extensionStates.map(s => [
						s.extensionId,
						s.targetState,
					]),
				)
	...
	async function handleOptionChange<
		K extends keyof Pick<StoredOptions, 'position' | 'showButtons'>,
	>(key: K, value: StoredOptions[K]): Promise<void> {
		savingOptionKey = key;
...
await matchOptions();
	...
	function navigateTo(view: ViewMode, profileId?: string): void {
		...
		if (profileId) {
			...
			if (profile)
initCustomProfileEditing(profile);
		}
	}
...
return;
		const newProfile = createProfileGroup(
			newProfileName.trim(),
			profiles.length,
		);
	...
	function updateDefaultExtensionState(
		extId: string,
...
validationErrors[`condition-${index}`] = error;
	...
	async function saveCustomProfile(): Promise<void> {
		if (!validateCustomProfile() || !selectedProfile)
return;
...
<div class="app-container">
	...
	<aside class="sidebar">
		...
		<nav class="sidebar-nav">
			...
			<button
				class="nav-item"
				class:active={currentView === 'settings-ui'}
			...
			<button
				class="nav-item"
				class:active={currentView === 'settings-general'}
			...
			<div class="nav-section-title">
				{getI18N('profileGroups') || 'Profile Groups'}
			</div>

			<button
				class="nav-item"
				class:active={currentView === 'profile-default'}
			...
			{#each profiles as profile, index (profile.id)}
				<button
					...
					onclick={() => navigateTo('profile-custom', profile.id)}
				>
		...
		<header class="content-header">
			<div class="header-breadcrumb">
				{#if currentView.startsWith('settings')}
					<span class="breadcrumb-item"
						>{getI18N('settings') || 'Settings'}</span
					>
					<span class="breadcrumb-separator"></span>
					<span class="breadcrumb-current">
						{currentView === 'settings-ui'
							? getI18N('settingsUI') || 'Interface'
							: getI18N('settingsGeneral') || 'General'}
					</span>
				{:else if currentView === 'profile-default'}
					<span class="breadcrumb-item"
						>{getI18N('profileGroups') || 'Profile Groups'}</span
					>
					<span class="breadcrumb-separator"></span>
					<span class="breadcrumb-current"
						>{getI18N('defaultProfile') || 'Default'}</span
					>
			...
			<div class="header-actions">
				{#if currentView === 'profile-default'}
					<button class="btn btn-primary" onclick={saveDefaultProfile}
						>{getI18N('save') || 'Save'}</button
					>
				{:else if currentView === 'profile-custom' && selectedProfile}
					<label class="toggle-switch">
						<input
							...
							onchange={() => handleToggleProfile(selectedProfile)}
						/>
						<span class="slider"></span>
					</label>
					<button class="btn btn-primary" onclick={saveCustomProfile}
						>{getI18N('save') || 'Save'}</button
					>
					<button
						class="btn btn-danger"
						onclick={() => handleDeleteProfile(selectedProfile.id)}
						>{getI18N('delete') || 'Delete'}</button
					>
				{/if}
			</div>
		</header>
		...
		<div class="content-body">
			{#if currentView === 'settings-ui'}
				<div class="page-header">
					<h2 class="page-title">
						{getI18N('settingsUI') || 'Interface Settings'}
					</h2>
					<p class="page-description">
						{getI18N('settingsUIDescription') ||
							'Customize how the extension manager looks.'}
					</p>
				</div>
				<div class="card">
					<h3 class="card-title">
						{getI18N('displayOptions') || 'Display Options'}
					</h3>
					<div class="form-group">
						<label class="form-label" for="showButtons"
							>{getI18N('showButtons') || 'Show Buttons'}</label
						>
						<select
							id="showButtons"
							class="select"
							value={optionShowButtons}
							onchange={e =>
								handleOptionChange(
									'showButtons',
									(e.target as HTMLSelectElement)
										.value as StoredOptions['showButtons'],
								)}
							disabled={savingOptionKey === 'showButtons'}
						>
							<option value="on-demand"
								>{getI18N('onRightClick') || 'On right-click (default)'}</option
							>
			...
			{:else if currentView === 'settings-general'}
				<div class="page-header">
					<h2 class="page-title">
						{getI18N('settingsGeneral') || 'General Settings'}
					</h2>
					<p class="page-description">
						{getI18N('settingsGeneralDescription') ||
							'Configure popup behavior and layout.'}
					</p>
				</div>
				<div class="card">
					<h3 class="card-title">
						{getI18N('popupBehavior') || 'Popup Behavior'}
					</h3>
					<div class="form-group">
						<label class="form-label" for="position"
							>{getI18N('position') || 'Position'}</label
						>
						<select
							id="position"
							class="select"
							...
							onchange={e =>
								handleOptionChange(
									'position',
									(e.target as HTMLSelectElement)
										.value as StoredOptions['position'],
								)}
							disabled={savingOptionKey === 'position'}
						>
							<option value="popup"
								>{getI18N('popupMenu') || 'Popup (default)'}</option
							>
							<option value="window"
								>{getI18N('popupWindow') || 'Window'}</option
							>
			...
			{:else if currentView === 'profile-default'}
				<div class="page-header">
					<h2 class="page-title">
						{getI18N('defaultProfile') || 'Default Profile'}
					</h2>
					<p class="page-description">
						{getI18N('defaultProfileDescription') ||
							'The default profile applies when no other profile matches.'}
					</p>
				</div>
				<div class="card">
					<h3 class="card-title">
						{getI18N('extensionStates') || 'Extension States'}
					</h3>
					...
					<div class="extension-states">
						<div class="extension-states-header" aria-hidden="true">
							<div class="state-legend">
								<span>{getI18N('keepState') || 'Keep'}</span>
								<span>{getI18N('enable') || 'Enable'}</span>
						...
						{#each filteredDefaultExtensions as ext (ext.id)}
							<div class="extension-state-item">
								...
								<div class="state-selector">
									<button
										class="state-btn"
										class:active={ext.targetState === 'keep'}
										onclick={() => updateDefaultExtensionState(ext.id, 'keep')}
										title={getI18N('keepState') || 'Keep'}
									></button>
									<button
										class="state-btn enable"
										class:active={ext.targetState === 'enable'}
										onclick={() =>
											updateDefaultExtensionState(ext.id, 'enable')}
										title={getI18N('enable') || 'Enable'}
									></button>
									<button
										class="state-btn disable"
										class:active={ext.targetState === 'disable'}
										onclick={() =>
											updateDefaultExtensionState(ext.id, 'disable')}
										title={getI18N('disable') || 'Disable'}
									></button>
								</div>
							</div>
						{/each}
			...
			{:else if currentView === 'profile-custom' && selectedProfile}
				<div class="page-header">
					<h2 class="page-title">
						<span class="sr-only"
							>{getI18N('profileName') || 'Profile Name'}</span
						>
						<input
							type="text"
							...
							aria-label={getI18N('profileName') || 'Profile Name'}
							placeholder={getI18N('profileNamePlaceholder') || 'Profile name'}
						/>
					</h2>
					{#if validationErrors.name}<div class="validation-error">
							{validationErrors.name}
						</div>{/if}
				</div>
				...
				<div class="card">
					...
					<div class="form-group inline-form">
						...
						<span class="form-help"
							>{getI18N('higherFirst') ||
								'Higher number = higher priority'}</span
						>
					</div>
				</div>
				...
				<div class="card">
					<h3 class="card-title">
						{getI18N('urlConditions') || 'URL Conditions'}
					</h3>
					<p class="card-description">
						{getI18N('urlConditionsHelp') ||
							'This profile will be applied when URL matches any condition.'}
					</p>
					{#if validationErrors.conditions}<div class="validation-error">
							{validationErrors.conditions}
						</div>{/if}
						...
						{#each editingConditions as condition, index (condition.id)}
							<div class="condition-item">
								...
								<input
									type="text"
									class="input"
									class:error={validationErrors[`condition-${index}`]}
							...
							{#if validationErrors[`condition-${index}`]}<div
							...
							<div class="extension-state-item">
								...
								<div class="state-selector">
									<button
										class="state-btn"
										class:active={ext.targetState === 'keep'}
										onclick={() => updateExtensionState(ext.id, 'keep')}
										title={getI18N('keepState') || 'Keep'}
									></button>
									<button
										class="state-btn enable"
										class:active={ext.targetState === 'enable'}
										onclick={() => updateExtensionState(ext.id, 'enable')}
										title={getI18N('enable') || 'Enable'}
									></button>
									<button
										class="state-btn disable"
										class:active={ext.targetState === 'disable'}
										onclick={() => updateExtensionState(ext.id, 'disable')}
										title={getI18N('disable') || 'Disable'}
									></button>
								</div>
							</div>
			...
			{:else}
				<EmptyState
					icon="📋"
					title={getI18N('noProfiles') || 'No Profile Selected'}
					description={getI18N('profilesDescription') ||
						'Profile groups can automatically enable/disable extensions based on the website you visit.'}
					buttonText={getI18N('addProfile') || 'New Profile'}
					onAction={openNewProfileModal}
				/>
			{/if}
		</div>
	</main>
...
<Modal
	bind:show={showNewProfileModal}
	title={getI18N('newProfile') || 'New Profile'}
>
	{#snippet children()}
		<div class="form-group">
			<label class="form-label" for="new-profile-name"
	...
	{#snippet footer()}
		<button
			...
			onclick={() => (showNewProfileModal = false)}
			>{getI18N('cancel') || 'Cancel'}</button
		>
		<button
			class="btn btn-primary"
			onclick={createNewProfile}
			disabled={!newProfileName.trim()}>{getI18N('create') || 'Create'}</button
		>
	{/snippet}
</Modal>
